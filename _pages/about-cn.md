---
permalink: /cn/
title: "孙雪丽"
author_profile: true
---

<span id="about"></span>
我目前是南京邮电大学物联网学院讲师、硕士生导师。我的研究方向聚焦于**网络可靠性**、**容错计算**、**面向故障诊断的图学习**及其在**数据中心网络**和**云计算**中的应用。我在IEEE/ACM Transactions on Networking **（CCF A类）**、IEEE Transactions on Computers **（CCF A类）**、IEEE Transactions on Reliability、The Computer Journal、Journal of Computer Science and Technology、Theoretical Computer Science等国际知名期刊和会议上发表**30篇论文**，其中**CCF B类及以上期刊论文14篇**（含CCF A类4篇）。

我于2023年6月获得苏州大学软件工程专业工学博士学位，师从樊建席教授。在博士期间，我荣获博士研究生国家奖学金🏆、两次校级特等奖学金，主持江苏省研究生科研创新计划项目（2021-2022）。

我是中国计算机学会（CCF）会员，并担任IEEE Transactions on Reliability、Theoretical Computer Science、The Computer Journal、Journal of Supercomputing等多个国际期刊的审稿人。

# 💡 研究生招生

我非常期待与有志于容错计算、数据中心网络架构和面向故障诊断的图学习研究的优秀学生合作。如果你对前沿研究充满好奇心、勤奋努力并渴望探索，我的团队将为你提供一个支持性的成长环境。

🚀 加入我的团队——让我们一起推进计算机系统的可靠性研究！

**研究方向：**
- 网络可靠性
- 容错计算
- 互连网络体系结构
- 数据中心网络
- 图学习

**我能提供：**
- 💰 科研经费支持参加学术会议
- 🎓 优秀学生可提前进入实验室学习（免试生和统招生）
- 📝 结合本科毕业设计提升科研能力
- 🏆 完整的科研奖励体系
- 🌍 优先支持国际学术交流机会
- ✈️ 推荐赴海外知名高校深造的机会

<span id="news"></span>
# 🔥 最新动态

- **2026.03**：🎉🎉 两篇论文（含一篇第一作者）分别被 IEEE Transactions on Computers 和 IEEE/ACM Transactions on Networking **(CCF A类顶级期刊)** 接收！
- **2025.12**：🎉🎉 两篇论文（含一篇第一作者）被 IEEE Transactions on Computers **(CCF A类顶级期刊)** 接收！
- **2025.12**：🎉 南京邮电大学新教师教学竞赛特等奖
- **2023.06**：🎉 入职南京邮电大学，担任讲师职位 ✨！
- **2023.06**：🎓 顺利通过苏州大学博士学位论文答辩！


<span id="publications"></span>
# 📝 论文发表

- **Xueli Sun**, S. Kan, W. Fan, J. Qi, Z. Dong, J. Fan: A Graph Neural Network Approach for Hybrid Node-Edge Fault Diagnosis in Interconnection Networks under the HPMC* Model. *IEEE/ACM Transactions on Networking* **(CCF A类)**, 34: 3918-3931 (2026).

- W. Fan, X. Peng, F. Xiao, M. Lv, **Xueli Sun**, W. Duan, S.-Y. Hsieh: A Highly Cost-Effective and Fault-Tolerant Network Topology for Large-Scale Data Centers. *IEEE Transactions on Computers* **(CCF A类)**, 75(4): 1488-1502 (2026).

- **Xueli Sun**, S. Kan, J. Fan, W. Fan, J. Qi, Z. Dong: Multi-Component Fault Tolerance and Path Construction in Interconnection Networks. *IEEE Transactions on Computers* **(CCF A类)**, 75(3): 1043-1057 (2026).

- X. Liu, Z. Dong, W. Fan, M. Lv, **Xueli Sun**, J. Qi, S.-Y. Hsieh: A Highly Reliable Multiplexing Scheme in Hypercube-Structured Hierarchical Networks. *IEEE Transactions on Computers* **(CCF A类)**, 74(10): 3462-3475 (2025).

- Y. Pan, W. Fan, M. Lv, **Xueli Sun**, S. Yu: An Incremental Scalable Network Architecture With Fault-Tolerant Communication. *IEEE Transactions on Reliability* **(SCI二区)**, 74(4): 5738-5752 (2025).

- X. Liu, M. Lv, W. Fan, **Xueli Sun**: Cluster connectivity and super cluster connectivity of half hypercube networks. *Theoretical Computer Science* **(CCF B类)**, 1011: 114715 (2024).

- X. Liu, M. Lv, W. Fan, **Xueli Sun**, Z. Dong, F. Xiao: Reliability of Half Hypercube Networks under Cluster Faults. *CSCWD* 2024: 2153-2158.

- J. Pei, M. Lv, W. Fan, **Xueli Sun**, X. He, F. Xiao: A protection routing with secure mechanism in the data center network WaveCube. *CSCWD* 2024: 2159-2164.

- Y. Xu, W. Fan, M. Lv, **Xueli Sun**, L. Han, F. Xiao: Parallel Construction of Independent Spanning Trees on 3-ary n-cube Networks. *NPC (1)* 2024: 133-145.

- D. Wang, W. Fan, F. Xiao, M. Lv, **Xueli Sun**, L. Han: Distributed Dynamic Virtual Network Embedding in Container Networks. *WASA (2)* 2024: 205-215.

- Y. Xu, W. Fan, M. Lv, **Xueli Sun**, X. Li, F. Xiao: An Efficient Fault-Tolerant Communication Scheme in 3-Ary n-Cube Networks. *WASA (2)* 2024: 342-354.

- **Xueli Sun**, J. Fan, B. Cheng, J. Zhou, Y. Wang: Relationship Between Component Connectivity And Component Diagnosability Of Some Regular Networks. *The Computer Journal* **(CCF B类)**, 66(8): 2033-2042 (2023).

- **Xueli Sun**, J. Fan, B. Cheng, Y. Wang, L. Zhang: Probabilistic Fault Diagnosis of Clustered Faults for Multiprocessor Systems. *Journal of Computer Science and Technology* **(CCF B类)**, 38(4): 821-833 (2023).

- **Xueli Sun**, J. Fan, B. Cheng, Y. Wang, J. Zhou: Reliability of Augmented 3-Ary n-Cubes with Extra Faults. *Journal of Interconnect Networks*, 23(2): 2150040:1-2150040:20 (2023).

- **Xueli Sun**, J. Fan, B. Cheng, Y. Wang: The t/m-diagnosis strategy of augmented k-ary n-cubes. *Theoretical Computer Science* **(CCF B类)**, 945: 113671 (2023).

- **Xueli Sun**, J. Fan, E. Sabir, B. Cheng, J. Yu: Reliability of augmented k-ary n-cubes under the extra connectivity condition. *Journal of Supercomputing* **(SCI三区)**, 79(12): 13641-13669 (2023).

- **Xueli Sun**, J. Fan, S. Kan, W. Fan, X. Jia: Component Reliability of a Class of Regular Networks and Its Applications. *IEEE Transactions on Reliability* **(SCI二区)**, 72(2): 637-650 (2023).

- Y. Wang, J. Fan, **Xueli Sun**, B. Cheng, Y. Wang: Fault-tolerability of the hypercube and variants with faulty subcubes. *Journal of Parallel and Distributed Computing* **(CCF B类)**, 167: 148-156 (2022).

- Y. Wang, **Xueli Sun**, W. Fan, B. Cheng, L. Xu, J. Fan: Relationship between g-extra Connectivity and g-restricted Connectivity in Networks. *ICPADS* 2022: 155-160.

- **Xueli Sun**, J. Fan, B. Cheng, Z. Liu, J. Yu: Component conditional fault tolerance of hierarchical folded cubic networks. *Theoretical Computer Science* **(CCF B类)**, 883: 44-58 (2021).

- **Xueli Sun**, S. Zhou, M. Lv, J. Liu, G. Lian: Intermittent Fault Diagnosability of Some General Regular Networks. *The Computer Journal* **(CCF B类)**, 63(1): 16-24 (2020).

- **Xueli Sun**, S. Zhou, Z. Gu, Y. Wang, M. Li: Reliability evaluation of complete cubic networks. *International Journal of Parallel, Emergent and Distributed Systems*, 35(1): 42-56 (2020).

- M. Lv, S. Zhou, J. Liu, **Xueli Sun**, G. Lian: Fault diagnosability of DQcube under the PMC model. *Discrete Applied Mathematics* **(CCF C类, SCI三区)**, 259: 180-192 (2019).

- M. Lv, S. Zhou, **Xueli Sun**, G. Lian, J. Liu: Reliability of (n, k)-star network based on g-extra conditional fault. *Theoretical Computer Science* **(CCF B类)**, 757: 44-55 (2019).

- **Xueli Sun**, Q. Dong, S. Zhou, M. Lv, G. Lian, J. Liu: Fault tolerance analysis of hierarchical folded cube. *Theoretical Computer Science* **(CCF B类)**, 790: 117-130 (2019).

- M. Lv, S. Zhou, **Xueli Sun**, G. Lian, J. Liu, D. Wang: Probabilistic diagnosis of clustered faults for hypercube-based multiprocessor system. *Theoretical Computer Science* **(CCF B类)**, 793: 113-131 (2019).

- M. Lv, S. Zhou, **Xueli Sun**, G. Lian, G. Chen: The g-good-neighbour conditional diagnosability of multiprocessor system based on half hypercube. *International Journal of Computer Mathematics: Computer Systems Theory*, 3(3): 160-176 (2018).

- M. Lv, S. Zhou, **Xueli Sun**, G. Lian, J. Liu: Reliability Evaluation of Data Center Network DCell. *Parallel Processing Letters*, 28(4): 1850015:1-1850015:10 (2018).

<span id="projects"></span>
# 🔬 科研项目

**主持项目：**
- 国家自然科学基金青年项目
- 江苏省高校面上自然科学基金项目
- 南京邮电大学人才启动经费专项

<span id="services"></span>
# 🌠 学术服务

**国际期刊审稿人：**
- IEEE Transactions on Reliability
- Theoretical Computer Science
- The Computer Journal
- Journal of Supercomputing

<span id="honors"></span>
# 🎖 荣誉奖项

- **2023年**：博士研究生国家奖学金（Top 1%）
- **2022年**：苏州大学特等奖学金（Top 1%）
- **2021年**：苏州大学特等奖学金（Top 1%）
- **2021-2022年**：江苏省研究生科研创新计划项目（主持）

<span id="education"></span>
# 📖 教育与工作经历

- **2023.06 - 至今**：讲师、硕士生导师
  - 物联网学院，南京邮电大学
  - 江苏省南京市

- **2020.09 - 2023.06**：工学博士，软件工程专业，苏州大学
  - 导师：樊建席 教授
  - 论文方向：网络可靠性与故障诊断

